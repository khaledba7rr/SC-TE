...
const AttributeValues: React.FC<HeaderProps> = ({ type, attributeValues, attribute, selectProductAttributes }) =>
{

    const [attributeSelection, setAttributeSelection] = useState<AttributeSelection[] | null>(null);
  
    const HandleAttributeSelection = (attributeId: number, valueId: number) =>
    {
        if (!attributeSelection)
        {
            const newAttributeSelection: AttributeSelection[] = [{ attribute_id: attributeId, value_id: valueId }];
            setAttributeSelection(newAttributeSelection);
        }
        else
        {
            const existingAttribute = attributeSelection.find((attr) => attr.attribute_id === attributeId);
            ...
            {
                setAttributeSelection([...attributeSelection, { attribute_id: attributeId, value_id: valueId }]);
            }
        }
    ...
    const GetSelectedAttribute = (attributeId: number) =>
    {
        ...
        const selectedAttribute = attributeSelection.find((attr) => attr.attribute_id === attributeId);
